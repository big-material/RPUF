...
def process_input_features_regression(inputs: dict, model_type: str) -> pd.DataFrame:
    ...
    if model_type in ["CS", "S_CS"]:
        base.update({"CS_rate": inputs["CS_rate × 10⁻⁴"], "CS_TD": CS_TD_MAP.get(inputs["CS_TD"], 0)})
...
def plot_all_distributions(results_dict):
    xlabel_units = {
        "AD": "kg/m$^3$",  
        "TC": r"$\times 10^{-3}$ (W/(m·K))",
        "S_TC": r"$\times 10^{-3}$ (W·K·m$^2$/kg)",
        "CS": "kPa",
        "S_CS": "kPa·m$^3$/kg",
    }

    fig, axes = plt.subplots(2, 3, figsize=(10, 7), dpi=600)
    axes = axes.flatten()
    ...
    for i, (feature_name, value) in enumerate(results_dict.items()):
        ...
        ax.plot(value, y_star, marker="*", markersize=12, color="red",
                label=f"Pred {value:.2f}")

        # 设置 xlabel
        if feature_name in ["TC", "S_TC"]:
            ax.set_xlabel(f"{feature_name} {xlabel_units[feature_name]}", fontsize=10)
        elif feature_name in xlabel_units:
            ax.set_xlabel(f"{feature_name} ({xlabel_units[feature_name]})", fontsize=10)
        else:
...
def main():
    ...
    st.title("🧪RPUF prediction for Density, Thermal conductivity & Compressive Strength")
    # st.markdown("---")
    ...
    with st.expander("Feature input area", expanded=True):
        ...
        with c4:
            ba1_type = st.selectbox("BA1", ["none", "H₂O", "physical"], key="sel_ba1")
        with c5:
            ba2_type = st.selectbox("BA2", ["none", "H₂O", "physical"], key="sel_ba2")
        ...
        numeric_groups = {
            "Composition": {
                ...
                "BA1_Phr": (0.0, 20.0, 3.0), 
                "BA2_Phr": (0.0, 50.0, 3.1),
    ...
    if st.button("🔮 Prediction"):
        ...
        inputs["BA_Phr"] = values["BA1_Phr"] + values["BA2_Phr"] 

        f_H2O = 0.0
        if ba1_type == "H₂O":                # 如果 BA1 是 H2O
            f_H2O += values["BA1_Phr"] / 18
        if ba2_type == "H₂O":                # 如果 BA2 是 H2O
            f_H2O += values["BA2_Phr"] / 18
        inputs["f(H2O)"] = f_H2O
        ...
        if ba1_type == "H₂O":
            M_loss += f_H2O * 44
        elif ba1_type == "physical":
            M_loss += values["BA1_Phr"]

        if ba2_type == "H₂O":
            M_loss += f_H2O * 44
        elif ba2_type == "physical":
            M_loss += values["BA2_Phr"]
        inputs["M_loss"] = M_loss

        # 动态计算 Yield
        inputs["Yield"] = ((100 + values["PO_Phr"] + inputs["BA_Phr"] + values["FS_Phr"] + values["Cata_Phr"] - inputs["M_loss"]) /
                        (100 + values["PO_Phr"] + inputs["BA_Phr"] + values["FS_Phr"] + values["Cata_Phr"])) * 100
        ...
        # =============== HD_LD 分类 =================
        if "HD_LD" in classification_models:
            try:
                proba = classification_models["HD_LD"].predict_proba(X_cls)[0]
                positive_prob = proba[1] if len(proba) >= 2 else proba[0]
                classification_results["HD_LD"] = {
                    "prob": float(positive_prob),
                    "label": "HD" if positive_prob > 0.5 else "LD"
                }
            except Exception as e:
                classification_results["HD_LD"] = {"prob": None, "label": "分类失败"}
                st.error(f"Classification model HD_LD prediction anomaly: {e}")
        ...
        for name in ["I", "II", "III", "IV"]:
            ...
            try:
                ...
                positive_prob = proba[1] if len(proba) >= 2 else proba[0]
                quadrant_probs[name] = float(positive_prob)
            except Exception as e:
                quadrant_probs[name] = 0.0
                st.error(f"Classification model {name} prediction anomaly: {e}")

        # === 选出概率最大的象限作为唯一正类 ===
        if quadrant_probs:
            predicted_quadrant = max(quadrant_probs, key=quadrant_probs.get)
        else:
            predicted_quadrant = None

        # === 构建象限结果字典 ===
        for name, prob in quadrant_probs.items():
            label = "1" if name == predicted_quadrant else "0"
            classification_results[name] = {"prob": prob, "label": label}
        ...
        for m in all_models:
            if m in classification_results:
                label = classification_results[m]["label"]
                # 针对 HD_LD 单独处理
                if m == "HD_LD":
                    if label == "1":
                        label = "HD"
                ...
                result_row.append(f"{regression_results[m]:.2f}" if isinstance(regression_results[m], float) else "-")
            else:
                result_row.append("-")
        ...
        '>
            <tr style='background-color: #4CAF50; color: white;'>
                <th>Label</th>
        """ + "".join([f"<th>{col}</th>" for col in all_models]) + "</tr>"

        html_table += "<tr style='background-color: #f2f2f2; color: black;'>"
        html_table += "<td>Result</td>" + "".join([f"<td style='border: 1px solid #ddd; padding: 8px;'>{val}</td>" for val in result_row])
        html_table += "</tr></table>"

        st.markdown(html_table, unsafe_allow_html=True)

        # 回归指标可视化
        st.subheader("📊 Predicted values in own distribution")
